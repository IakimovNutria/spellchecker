...
from most_common_words.utils import get_most_common_words, get_most_common_words_dict, get_colored_diff
    ...
    if word not in most_common_words_set:
        for common_word in most_common_words_list:
            if fuzz.ratio(word, common_word) >= percent:
                print(f'{word} invalid, correct word: {get_colored_diff(word, common_word)}', file=output)
                break
        else:
            print(f"{word} invalid. Don't know how to correct a word with precision {percent}", file=output)
    else:
...
parser.add_argument("-i", "--input", type=str, metavar='FILENAME', required=False,
                    help="input file to read and spell check")
parser.add_argument("-o", "--output", type=str, metavar='FILENAME', required=False,
